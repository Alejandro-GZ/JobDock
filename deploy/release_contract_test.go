...
func TestReleaseWorkflowPublishesCompleteVersionedSet(t *testing.T) {
	...
	for _, required := range []string{
		...
		"release-assets/docker-compose.domain.yml",
		"release-assets/docker-compose.proxy.yml",
		"release-assets/docker-compose.local.yml",
	...
	for _, required := range []string{"release-manifest.json", "docker-compose.yml", "docker-compose.domain.yml", "docker-compose.proxy.yml", "docker-compose.local.yml", "Caddyfile", "install-control-plane.sh", "install-agent.sh", "jobdock-doctor", "SHA256SUMS", "## Highlights", "## Changes", "sha256sum", "pip install jobdock-sdk==", "SDK wheel checksum mismatch", "SDK sdist checksum mismatch"} {
...
func TestReleaseInstallationIsDocumentedSeparatelyFromDevelopment(t *testing.T) {
	...
	for _, required := range []string{
		...
		"--mode domain --domain dock.example.com",
		"verifies `SHA256SUMS`",
		"/etc/jobdock",
		"/var/lib/jobdock",
