...
2. Start the control plane with `docker compose -f deploy/docker-compose.yml up --build`.
...
6. Start a CPU agent with `docker compose -f deploy/docker-compose.agent.yml up -d --build`, or add `-f deploy/docker-compose.agent.gpu.yml` for NVIDIA discovery.
