...
8. Every GitHub Release contains `release-manifest.json`, a digest-pinned `docker-compose.yml`, the version-pinned `install-control-plane.sh` bootstrap, a digest-pinned `install-agent.sh`, and the already validated SDK wheel and sdist. One `SHA256SUMS` covers the manifest, deployment files, both installers, wheel, and sdist. The release notes include the one-command bootstrap and exact `pip install jobdock-sdk==VERSION` command.
