...
6. After digest verification, one job assembles the final release bundle exactly once. The release E2E then starts an empty Linux Docker-in-Docker host with only Docker, Compose, curl, and standard system utilities. That host downloads the bundle over HTTP, runs the published `install-control-plane.sh`, validates health with `jobdock-doctor --json`, creates an enrollment token through the public API, and runs the published `install-agent.sh`. The gate completes an existing-OCI job, a Railpack/BuildKit source build, a managed-image job, and a control-plane restart that proves persisted builds and jobs remain available. The repository is not mounted into the clean host and no JobDock component is compiled there.
...
8. Every GitHub Release publishes the exact bundle that passed the clean-host gate; downstream jobs never regenerate it. It contains `release-manifest.json`, a digest-pinned base `docker-compose.yml`, versioned domain/proxy/local Compose overlays, the Caddy configuration, the version-pinned `install-control-plane.sh` bootstrap, the read-only `jobdock-doctor`, the backup/restore `jobdockctl`, a digest-pinned `install-agent.sh`, the checksum-verifying `install-cli.sh`, the precompiled CLI archives, and the already validated SDK wheel and sdist. One `SHA256SUMS` covers the complete set. The release notes include CLI and control-plane installation plus the exact `pip install jobdock-sdk==VERSION` command.
9. On the first release, set all three package visibilities to public in the repository package settings.
10. QEMU-backed release smoke tests execute the arm64 server, CPU agent, and CLI and verify their embedded version. The amd64 clean-host gate exercises the complete builder-capable installation path. Any bootstrap, doctor, enrollment, workload, source-build, persistence, or cleanup-visible failure blocks publication of the GitHub Release.
