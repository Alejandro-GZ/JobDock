...
8. Every GitHub Release contains `release-manifest.json`, a digest-pinned base `docker-compose.yml`, versioned domain/proxy/local Compose overlays, the Caddy configuration, the version-pinned `install-control-plane.sh` bootstrap, the read-only `jobdock-doctor`, the backup/restore `jobdockctl`, a digest-pinned `install-agent.sh`, and the already validated SDK wheel and sdist. One `SHA256SUMS` covers the manifest, deployment files, commands, wheel, and sdist. The release notes include the one-command bootstrap and exact `pip install jobdock-sdk==VERSION` command.
