...
export const api={
  ...
  saveDashboard:(id:string,widgets:DashboardWidget[],materializedFrom?:DashboardTemplateReference|null,dashboardID?:string)=>request<DashboardPreference>(`/jobs/${id}/${dashboardID?`dashboards/${dashboardID}`:"dashboard"}`,{method:"PUT",body:JSON.stringify({schema_version:dashboardSchemaVersion,widgets,...(materializedFrom===undefined?{}:{materialized_from:materializedFrom})})}),
