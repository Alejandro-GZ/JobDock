...
export const api={
  ...
  logs:async(id:string,stream:"stdout"|"stderr")=>{const response=await fetch(`/api/v1/jobs/${id}/logs/${stream}`);if(!response.ok)throw new APIError(response.status,"logs_failed","Unable to load logs");return response.text()},
  nodes:async()=> (await request<{items:Node[]|null}>("/nodes")).items??[],enrollmentToken:()=>request<{token:string;expires_at:string}>("/nodes/enrollment-tokens",{method:"POST"}),setNode:(id:string,action:"drain"|"resume")=>request(`/nodes/${id}/${action}`,{method:"POST"}),
