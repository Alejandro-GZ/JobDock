...
export const api={
  ...
  observabilityCatalog:async(id:string,attemptId?:string)=>{const query=attemptId?`?attempt_id=${encodeURIComponent(attemptId)}`:"";return request<{attempt_id:string;items:import("./types").ObservableSourceDescriptor[];phases:import("./types").ObservabilityPhase[]}>(`/jobs/${id}/observability/catalog${query}`)},
  ...
  dashboards:(id:string)=>request<DashboardList>(`/jobs/${id}/dashboards`),
  dashboardByID:(id:string,dashboardID:string)=>request<DashboardPreference>(`/jobs/${id}/dashboards/${dashboardID}`),
  createDashboard:(id:string,name:string,sourceDashboardID?:string)=>request<DashboardPreference>(`/jobs/${id}/dashboards`,{method:"POST",body:JSON.stringify({name,source_dashboard_id:sourceDashboardID||undefined,make_active:true})}),
  updateDashboard:(id:string,dashboardID:string,update:{name?:string;active?:boolean;is_default?:boolean})=>request<DashboardPreference>(`/jobs/${id}/dashboards/${dashboardID}`,{method:"PATCH",body:JSON.stringify(update)}),
  deleteDashboard:(id:string,dashboardID:string)=>request<void>(`/jobs/${id}/dashboards/${dashboardID}`,{method:"DELETE"}),
  saveDashboard:(id:string,widgets:DashboardWidget[],materializedFrom?:DashboardTemplateReference|null,dashboardID?:string,appearance?:DashboardAppearance|null)=>request<DashboardPreference>(`/jobs/${id}/${dashboardID?`dashboards/${dashboardID}`:"dashboard"}`,{method:"PUT",body:JSON.stringify({schema_version:dashboardSchemaVersion,widgets,...(appearance===undefined?{}:{appearance}),...(materializedFrom===undefined?{}:{materialized_from:materializedFrom})})}),
