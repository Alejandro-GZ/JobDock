...
import { buildFormData } from "@/lib/builds";
let csrfToken="";
...
export const api={
  ...
  createBuild:(name:string,source:File)=>request<Build>("/builds",{method:"POST",body:buildFormData(name,source)}),buildPlan:(id:string)=>request<BuildPlan>(`/builds/${id}/plan`),confirmBuild:(id:string)=>request<BuildPlan>(`/builds/${id}/confirm`,{method:"POST"}),cancelBuild:(id:string)=>request<Build>(`/builds/${id}/cancel`,{method:"POST"}),
