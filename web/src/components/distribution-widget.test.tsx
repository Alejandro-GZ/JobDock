...
describe("DistributionWidget",()=>{for(const type of ["histogram","boxplot","violin"] as const)it(`renders ${type} from the reusable summary`,()=>{const view=render(<DistributionWidget type={type} items={items}/>);expect(screen.getByLabelText(`${type} distribution`)).toBeTruthy();expect(screen.getByText(/residual/)).toBeTruthy();view.unmount()})});
