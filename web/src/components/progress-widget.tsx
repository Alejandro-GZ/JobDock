...
export function ProgressWidget({ state }: { state: ProgressState }) {
  ...
  return <section className="rounded-md border bg-card p-3" aria-label="Job progress">
    <div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-medium">Progress</h3>{primary != null && <span className="font-mono text-sm font-semibold">{Math.round(primary * 100)}%</span>}</div>
