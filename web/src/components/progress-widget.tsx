...
export function ProgressWidget({ state, actions, appearance }: { state: ProgressState; actions?: ReactNode; appearance?: DashboardWidgetAppearance }) {
  ...
  const density=appearance?.density??"normal",padding=density==="compact"?"p-2":density==="comfortable"?"p-5":"p-3",spacing=density==="compact"?"mt-1":"mt-3";
  return <section className={`h-full min-h-0 min-w-0 overflow-auto rounded-md border bg-card ${padding}`} aria-label="Job progress">
    ...
    {state.current && <div className={spacing}><div className="mb-1 flex justify-between text-xs"><span><span className="text-muted-foreground">Current stage · </span>{state.current.milestone || "Current stage"}</span><span>{Math.round(state.current.value * 100)}%</span></div><Progress value={state.current.value * 100} className="h-1.5" aria-label="Current stage progress"/></div>}
