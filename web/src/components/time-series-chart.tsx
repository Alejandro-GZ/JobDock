...
export function TimeSeriesChart({ title, points, color = "var(--primary)", format = compact, summary: suppliedSummary }: { title: string; points: SeriesPoint[]; color?: string; format?: (value: number) => string; summary?: { last: number; min: number; max: number } }) {
  ...
  const [zoom, setZoom] = useState<[number, number] | null>(null), [hover, setHover] = useState<SeriesPoint | null>(null), [statisticsPinned, setStatisticsPinned] = useState(false), [statisticsHovered, setStatisticsHovered] = useState(false);
  ...
  return <section className="flex min-h-0 flex-col rounded-md border bg-card p-2.5">
    ...
    {rangedPoints.length === 0 ? <div className="grid min-h-0 flex-1 place-items-center text-sm text-muted-foreground">No samples in this range</div> : <div ref={plotRef} className="relative min-h-0 flex-1 overflow-hidden"><svg role="img" aria-label={`${title} time series with ${rangedPoints.length} points`} width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="block size-full touch-none" onWheel={onWheel} onPointerMove={onPointerMove} onPointerLeave={() => setHover(null)}>
      {[0, .5, 1].map(ratio => <g key={ratio}><line x1={left} x2={width - right} y1={top + ratio * (height - top - bottom)} y2={top + ratio * (height - top - bottom)} className="stroke-border" strokeDasharray="3 4"/><text x={left - 7} y={top + ratio * (height - top - bottom) + 4} textAnchor="end" className="fill-muted-foreground text-[10px]">{format(maximum - ratio * span)}</text></g>)}<path d={path} fill="none" stroke={color} strokeWidth="2" vectorEffect="non-scaling-stroke"/>{hover && <><line x1={x(hover.timestamp)} x2={x(hover.timestamp)} y1={top} y2={height - bottom} className="stroke-muted-foreground" strokeDasharray="3 3"/><circle cx={x(hover.timestamp)} cy={y(hover.value)} r="4" fill={color}/></>}<text x={left} y={height - 5} className="fill-muted-foreground text-[10px]">{shortTime(domain[0])}</text><text x={width - right} y={height - 5} textAnchor="end" className="fill-muted-foreground text-[10px]">{shortTime(domain[1])}</text>
