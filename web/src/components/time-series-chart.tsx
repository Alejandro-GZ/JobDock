...
export function TimeSeriesChart({ title, points, markers = [], color = "var(--primary)", format = compact, summary: suppliedSummary, actions, range="all" }: { title: string; points: SeriesPoint[]; markers?: ChartMarker[]; color?: string; format?: (value: number) => string; summary?: { last: number; min: number; max: number }; actions?: ReactNode;range?:TimeRange }) {
  const plotRef = useRef<HTMLDivElement>(null), [plotSize, setPlotSize] = useState({ width: fallbackWidth, height: fallbackHeight });
  const rangedPoints = useMemo(() => chartRangePoints(points, range), [points, range]);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      const bounds = element.getBoundingClientRect(), width = Math.max(80, Math.round(bounds.width)), height = Math.max(48, Math.round(bounds.height));
  ...
  return <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-md border bg-card p-2.5">
