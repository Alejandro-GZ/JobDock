...
export function DistributionWidget({type,title,items}:{type:"histogram"|"boxplot"|"violin";title?:string;items:DistributionObservation[]}){
  ...
  const left=54,right=24,top=30,bottom=38,plotWidth=Math.max(1,size.width-left-right),plotHeight=Math.max(1,size.height-top-bottom),plotBottom=top+plotHeight,x=(value:number)=>left+(value-min)/span*plotWidth;
  const laneHeight=plotHeight/Math.max(1,items.length),laneY=(index:number)=>top+(index+.5)*laneHeight;
...
function format(value:number){return new Intl.NumberFormat(undefined,{maximumFractionDigits:2,notation:Math.abs(value)>=10000?"compact":"standard"}).format(value)}
