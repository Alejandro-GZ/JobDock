...
import { Activity, Boxes, ChevronDown, KeyRound, LayoutDashboard, Menu, Moon, Plus, ScrollText, Server, Sun, Users } from "lucide-react";
...
export function AppShell({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  <div className="lg:pl-56"><header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b bg-background/90 px-4 backdrop-blur lg:px-6"><Button variant="ghost" size="icon" className="lg:hidden" onClick={()=>setMobile(true)}><Menu className="size-5"/><span className="sr-only">Open navigation</span></Button><div className="ml-auto"><Button asChild size="sm"><NavLink to="/jobs/new"><Plus className="size-4"/>New job</NavLink></Button></div></header><main className="mx-auto max-w-[1500px] p-4 lg:p-6"><Outlet/></main></div></div>;
}
