...
import { ChevronLeft, ChevronRight, Eraser, GripVertical, Maximize2, PaintBucket, Paintbrush, Plus, Settings2, Terminal, Trash2, TriangleAlert } from "lucide-react";
...
type Props = {
    ...
    editMode?: boolean;
    sourceWarnings?: Array<{ widgetID?: string; source?: string; message: string }>;
};
...
export function ObservabilityDashboard({ jobID, attemptID, ready, numericSources, observableSources = [], progress, matrices, distributions = [], markers, initialWidgets = null, dashboardAppearance, onDashboardAppearanceChange, onWidgetsChange, onWidgetsReady, replacement, editMode = false, sourceWarnings = [] }: Props) {
    ...
    {!ready ? <div className="h-full animate-pulse rounded-md bg-muted/30"/> : <div className="flex h-full min-h-0 overflow-hidden bg-background">
      ...
      <div className={cn("relative min-w-0 flex-1 overflow-auto", editMode && "p-2 bg-[linear-gradient(to_right,hsl(var(--border)/.22)_1px,transparent_1px),linear-gradient(to_bottom,hsl(var(--border)/.22)_1px,transparent_1px)] bg-[size:8.333%_8.333%]")} style={{background:paintTarget?.widgetID==="dashboard"&&appearanceDrag?appearancePreview(appearanceDrag):paletteByRef(dashboardAppearance?.palette)?.surface?.background}} onDragOver={event => { if (event.dataTransfer.types.includes("text/jobdock-dashboard-palette")) {
        ...
        {ordered.length === 0 ? <EmptyDashboard editing={editMode}/> : <div data-dashboard-grid className="grid h-full min-h-0 grid-cols-12 grid-rows-12 auto-rows-fr gap-1">{ordered.map(widget => {const warning=sourceWarnings.find(item=>item.widgetID===widget.id||widget.sources.some(source=>item.source===`${source.kind}:${source.name}`));return <div key={widget.id} data-widget-id={widget.id} data-widget-type={widget.type} data-position={`${widget.position.x},${widget.position.y}`} data-size={`${widget.size.columns}x${widget.size.rows}`} style={{ gridColumn: `span ${widget.size.columns}`, gridRow: `span ${widget.size.rows}` }} className={cn("relative min-h-0 min-w-0 transition-[opacity,transform]", dragging === widget.id && "opacity-45")} onDragOver={event => { event.stopPropagation(); if(event.dataTransfer.types.includes("text/jobdock-dashboard-palette")){event.preventDefault();setPaintTarget({widgetID:"dashboard"});}
            ...
            return;
        } drop(event); }}>{warning&&<span role="note" title={warning.message} className="absolute right-1 top-1 z-40 grid size-5 place-items-center rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-300"><TriangleAlert className="size-3.5"/><span className="sr-only">Export note: {warning.message}</span></span>}{editMode&&paintTarget?.widgetID===widget.id&&!paintTarget.seriesKey&&appearanceDrag&&<div data-paint-preview="widget" className="pointer-events-none absolute inset-0 z-30 grid place-items-center rounded-md border-2 border-current text-foreground shadow-inner" style={{background:appearancePreview(appearanceDrag)}}><span className="rounded-full bg-background/85 p-2 shadow"><PaintBucket className="size-5"/></span></div>}{editMode ? <EditWidgetShell widget={widget} selected={selectedWidget === widget.id} appearanceDrag={appearanceDrag} paintTarget={paintTarget} onPaintTarget={setPaintTarget} onSelect={() => setSelectedWidget(widget.id)} onDragStart={event => { if ((event.target as HTMLElement).closest("button") || (event.target as HTMLElement).closest("[data-series-target]")) {
            event.preventDefault();
            return;
...
function DashboardWidgetView({ jobID, attemptID, widget, numericSources, sourceOptions, progress, matrices, distributions, markers, onUpdate }: {
    ...
    useEffect(() => { const host = [...document.querySelectorAll<HTMLElement>("[data-widget-id]")].find(element => element.dataset.widgetId === widget.id); if (!host)
        return; const surface = paletteByRef(dashboardAppearance?.palette)?.surface, appearance = widget.appearance; host.classList.add("dashboard-widget-surface"); host.dataset.widgetPadding = appearance?.padding ?? "normal"; host.dataset.widgetBorder = appearance?.border_style ?? (appearance?.border_color ? "subtle" : "none"); host.style.setProperty("--widget-bg", appearance?.background_color ?? surface?.card ?? ""); host.style.setProperty("--widget-border", appearance?.border_color ?? surface?.border ?? ""); host.style.setProperty("--widget-text", appearance?.text_color ?? surface?.text ?? ""); host.style.setProperty("--widget-accent", appearance?.accent_color ?? effectiveColors(dashboardAppearance, appearance)[0]); return () => { host.classList.remove("dashboard-widget-surface"); delete host.dataset.widgetPadding; delete host.dataset.widgetBorder; for (const property of ["--widget-bg", "--widget-border", "--widget-text", "--widget-accent"])
