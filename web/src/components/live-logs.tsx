...
export function LiveLogs({ jobId, attemptId, streams=["stdout","stderr"], embedded=false, actions, appearance }: { jobId:string;attemptId:string;streams?:StreamName[];embedded?:boolean;actions?:ReactNode;appearance?:DashboardWidgetAppearance }) {
  ...
  const style={"--log-font-size":`${appearance?.font_size??12}px`,"--stdout-color":appearance?.stdout_color??"#f4f4f5","--stderr-color":appearance?.stderr_color??"#f87171","--log-wrap":appearance?.wrap_lines===false?"pre":"pre-wrap"} as import("react").CSSProperties;
...
const LogChunk=memo(function LogChunk({fragment}:{fragment:LogFragment}){return <span data-log-stream={fragment.stream} className={fragment.stream==="stderr"?"text-red-300":undefined}>{fragment.text}</span>});
