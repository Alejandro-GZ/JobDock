...
import { offlineDashboardLogFragments } from "@/lib/offline-dashboard-data";
...
export function LiveLogs({ jobId, attemptId, streams=["stdout","stderr"], embedded=false, actions, appearance }: { jobId:string;attemptId:string;streams?:StreamName[];embedded?:boolean;actions?:ReactNode;appearance?:DashboardWidgetAppearance }) {
  const snapshot=offlineDashboardLogFragments(attemptId,streams),offline=snapshot!==undefined,initial=snapshot?.map((fragment,id)=>({...fragment,id}))??[];
  const [fragments,setFragments]=useState<LogFragment[]>(initial),[truncated,setTruncated]=useState(false),sequence=useRef(initial.length),output=useRef<HTMLPreElement>(null);
  useEffect(()=>{const next=offlineDashboardLogFragments(attemptId,streams)?.map((fragment,id)=>({...fragment,id}))??[];setFragments(next);setTruncated(false);sequence.current=next.length},[jobId,attemptId,streams.join(":"),offline]);
  ...
  const wantsCombined=streams.includes("stdout")&&streams.includes("stderr"),combined=useCombinedLogStream(jobId,attemptId,wantsCombined&&!offline,append),useFallback=!offline&&wantsCombined&&combined.fallback;
  const stdout=useLogStream(jobId,attemptId,"stdout",!offline&&streams.includes("stdout")&&(!wantsCombined||useFallback),append),stderr=useLogStream(jobId,attemptId,"stderr",!offline&&streams.includes("stderr")&&(!wantsCombined||useFallback),append);
  useEffect(()=>{if(output.current)output.current.scrollTop=output.current.scrollHeight},[fragments]);
  const states=wantsCombined&&!useFallback?[combined.connection]:streams.map(stream=>stream==="stdout"?stdout:stderr),live=!offline&&states.length>0&&states.every(state=>state==="live");
  const status=offline?"Snapshot":live?"Live":states.some(state=>state==="reconnecting")?"Reconnecting":"Connecting",statusTitle=offline?"Output captured when this offline report was generated":live?"Receiving incremental log updates":"The stream will reconnect automatically";
  const console=<div data-log-console data-log-mode={offline?"snapshot":"live"} className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border bg-zinc-950"><div data-log-toolbar className="pointer-events-none absolute inset-x-0 top-0 z-10 flex h-6 items-center border-b border-zinc-700/70 bg-zinc-900/95 px-2 text-[10px] text-zinc-200 shadow-sm backdrop-blur-sm"><span className="font-medium">Logs</span>{actions&&<span className="pointer-events-auto ml-1">{actions}</span>}<span tabIndex={0} className={live?"ml-auto flex items-center gap-1 text-emerald-400":"ml-auto flex items-center gap-1 text-amber-400"} title={statusTitle}>{live?<Radio className="size-3"/>:<AlertCircle className="size-3"/>}{status}</span></div>{truncated&&<p className="z-0 mt-6 border-b bg-amber-500/10 px-3 py-1.5 text-xs text-amber-300">Earlier output is hidden to keep the browser responsive. The complete log remains available in the ZIP.</p>}<pre ref={output} aria-label="Log output" className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap p-3 pt-8 font-mono text-xs text-zinc-100">{fragments.length?fragments.map(fragment=><LogChunk key={fragment.id} fragment={fragment}/>):"No output yet."}</pre></div>;
