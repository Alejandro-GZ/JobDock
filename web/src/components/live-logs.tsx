...
export function LiveLogs({ jobId, attemptId, streams=["stdout","stderr"], embedded=false, actions }: { jobId:string;attemptId:string;streams?:StreamName[];embedded?:boolean;actions?:ReactNode }) {
  ...
  const console=<div data-log-console className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border bg-zinc-950"><div data-log-toolbar className="pointer-events-none absolute inset-x-1 top-1 z-10 flex h-6 items-center rounded bg-zinc-900/90 px-2 text-[10px] text-zinc-200 shadow-sm backdrop-blur-sm"><span className="font-medium">Logs</span>{actions&&<span className="pointer-events-auto ml-1">{actions}</span>}<span tabIndex={0} className={live?"ml-auto flex items-center gap-1 text-emerald-400":"ml-auto flex items-center gap-1 text-amber-400"} title={live?"Receiving incremental log updates":"The stream will reconnect automatically"}>{live?<Radio className="size-3"/>:<AlertCircle className="size-3"/>}{status}</span></div>{truncated&&<p className="z-0 mt-8 border-b bg-amber-500/10 px-3 py-1.5 text-xs text-amber-300">Earlier output is hidden to keep the browser responsive. The complete log remains available in the ZIP.</p>}<pre ref={output} aria-label="Log output" className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap p-3 pt-9 font-mono text-xs text-zinc-100">{fragments.length?fragments.map(fragment=><LogChunk key={fragment.id} fragment={fragment}/>):"No output yet."}</pre></div>;
...
const LogChunk=memo(function LogChunk({fragment}:{fragment:LogFragment}){return <span className={fragment.stream==="stderr"?"text-red-300":undefined}>{fragment.text}</span>});
