...
export function FeatureImportanceWidget({jobID,attemptID,widget,onUpdate}:{jobID:string;attemptID:string;widget:DashboardWidget;onUpdate:(widget:DashboardWidget)=>void}){
  ...
  return <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-md border bg-card p-2" aria-label="Feature importance">
    ...
    <div ref={host} className="min-h-0 flex-1 overflow-auto"><div className="grid min-h-full gap-1" style={{gridTemplateRows:`repeat(${items.length}, minmax(18px, 1fr))`,height:Math.max(viewport.height,items.length*18)}}>{items.map(item=>{const width=Math.abs(item.value)/extent*48,left=item.value<0?50-width:50;return <div key={item.feature} className="grid grid-cols-[minmax(5rem,28%)_1fr_4rem] items-center gap-2 text-[10px]"><span className="truncate text-right" title={item.feature}>{item.feature}</span><div className="relative h-[40%] min-h-2.5 max-h-6 rounded-sm bg-muted"><i className="absolute inset-y-0 left-1/2 w-px bg-border"/><i className="absolute inset-y-0 rounded-sm" style={{left:`${left}%`,width:`${width}%`,background:item.value<0?"#dc2626":widget.appearance?.accent_color??"#2563eb"}}/></div><span className="truncate tabular-nums" title={`${item.value} ${unit}`}>{format(item.value)}{unit?` ${unit}`:""}</span></div>})}</div></div>
