...
export type TopologyAnchor = { x: number; panelX: number; y: number; side: "top" | "bottom" };
export type TopologySelection = ({ kind: "resource"; node: ResourceNode; href: string } | { kind: "job"; job: Job; href: string }) & { anchor: TopologyAnchor };
type TopologyData = { kind: "resource" | "dock" | "job"; label: string; href?: string; status?: string; heartbeat?: string; node?: ResourceNode; job?: Job; unseen?: boolean; inspected?: boolean; onSelect?: () => void; onOpen?: () => void };
...
function TopologyCard({ data }: NodeProps<TopologyNode>) {
  const selectable = Boolean(data.node || data.job);
  return <Tooltip><TooltipTrigger asChild><div role={selectable ? "button" : undefined} tabIndex={selectable ? 0 : -1} onKeyDown={event => { if ((event.key === "Enter" || event.key === " ") && data.onSelect) { event.preventDefault(); data.onSelect(); } if (event.key === "Enter" && (event.ctrlKey || event.metaKey)) data.onOpen?.(); }} className={cn("relative min-w-44 cursor-grab rounded-xl border-2 bg-background/95 px-3 py-2 shadow-lg outline-none transition-[transform,border-color,box-shadow] duration-300 active:cursor-grabbing focus-visible:ring-2 focus-visible:ring-ring", data.kind === "dock" && "min-w-32 border-primary bg-primary/5", border[data.status ?? ""], data.inspected && "ring-2 ring-primary ring-offset-2 ring-offset-background")}>
...
export function ResourceTopology({ resourceNodes, jobs, userId, seen, onSelectionChange }: { resourceNodes: ResourceNode[]; jobs: Job[]; userId: string; seen: Record<string, string>; onSelectionChange?: (selection?: TopologySelection) => void }) {
  const navigate = useNavigate(), topology = useMemo(() => buildTopology(resourceNodes, jobs, userId, seen), [resourceNodes, jobs, userId, seen]);
  const canonical = useMemo(() => new Map(topology.nodes.map(node => [node.id, node.position])), [topology.nodes]);
  const [flowNodes, setFlowNodes] = useState<TopologyNode[]>(topology.nodes), [inspectedID, setInspectedID] = useState("");
  const selectionFor = (node: TopologyNode, anchor: TopologyAnchor): TopologySelection | undefined => node.data.node && node.data.href ? { kind: "resource", node: node.data.node, href: node.data.href, anchor } : node.data.job && node.data.href ? { kind: "job", job: node.data.job, href: node.data.href, anchor } : undefined;
  const inspect = (node: TopologyNode, anchor = nodeAnchor(node.id)) => { const selection = selectionFor(node, anchor); if (!selection) return; setInspectedID(node.id); onSelectionChange?.(selection); };
  useEffect(() => setFlowNodes(topology.nodes.map(node => ({ ...node, data: { ...node.data, inspected: node.id === inspectedID, onSelect: () => inspect(node), onOpen: () => node.data.href && navigate(node.data.href) } }))), [topology, inspectedID]);
  ...
  return <div className="h-full min-h-0 w-full overflow-hidden bg-muted/10"><ReactFlow nodes={flowNodes} edges={topology.edges} nodeTypes={nodeTypes} onNodesChange={(changes: NodeChange<TopologyNode>[]) => setFlowNodes(nodes => applyNodeChanges(changes, nodes))} onNodeClick={(event, node) => inspect(node, elementAnchor(event.target))} onNodeDoubleClick={(_, node) => { if (node.data.href) navigate(node.data.href); }} onPaneClick={() => { setInspectedID(""); onSelectionChange?.(); }} onNodeDrag={(_, node) => clamp(node)} onNodeDragStop={(_, node) => reset(node)} fitView fitViewOptions={{ padding: .22 }} minZoom={.45} maxZoom={1.5} nodesConnectable={false} deleteKeyCode={null} proOptions={{ hideAttribution: true }}><Background gap={24} size={1}/><Controls showInteractive={false} position="bottom-right"/></ReactFlow></div>;
}

function elementAnchor(target: EventTarget | null): TopologyAnchor {
  const element = target instanceof Element ? target.closest(".react-flow__node") : null;
  return anchorFromRect(element?.getBoundingClientRect());
}

function nodeAnchor(id: string): TopologyAnchor {
  return anchorFromRect(document.querySelector(`[data-id="${id}"]`)?.getBoundingClientRect());
}

function anchorFromRect(rect?: DOMRect): TopologyAnchor {
  if (!rect) return { x: window.innerWidth / 2, panelX: window.innerWidth / 2, y: window.innerHeight / 2, side: "top" };
  const side = rect.top > 190 ? "top" : "bottom";
  const x = rect.left + rect.width / 2, margin = window.innerWidth >= 640 ? 320 : window.innerWidth / 2;
  return { x, panelX: Math.max(margin, Math.min(window.innerWidth - margin, x)), y: side === "top" ? rect.top : rect.bottom, side };
}
