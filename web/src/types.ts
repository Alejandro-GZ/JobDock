...
export type CPUPackage={id:string;model:string;physical_cores:number;logical_cpus:number[];total_millis:number;allocated_millis:number};
export type GPUDiscovery={status:"available"|"unavailable"|"disabled"|"unknown";error_code?:string;message?:string};
export type Node={id:string;name:string;status:string;agent_version:string;architecture:string;docker_version:string;cpu_total_millis:number;cpu_allocated_millis:number;memory_total_bytes:number;memory_allocated_bytes:number;workspace_free_bytes:number;labels:Record<string,string>;capabilities:string[];cpu_packages:CPUPackage[];gpus:GPU[];gpu_discovery:GPUDiscovery;last_heartbeat:string};
