...
export function Builds(){
  ...
  return <div className="space-y-4">
    <div className="overflow-hidden rounded-md border"><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Status</TableHead><TableHead>Source</TableHead><TableHead>Created</TableHead><TableHead className="text-right"><Button asChild size="sm"><Link to="/builds/new"><Plus className="size-4"/>Analyze project</Link></Button></TableHead></TableRow></TableHeader><TableBody>{(builds.data??[]).map(build=><TableRow key={build.id} className="cursor-pointer"><TableCell><Link className="font-medium hover:underline" to={`/builds/${build.id}`}>{build.name}</Link><div className="font-mono text-xs text-muted-foreground">{build.id.slice(0,8)}</div></TableCell><TableCell><BuildStatusBadge status={build.status}/></TableCell><TableCell><div>{build.source.filename}</div><div className="text-xs text-muted-foreground">{formatBytes(build.source.size)}</div></TableCell><TableCell className="text-muted-foreground">{new Date(build.created_at).toLocaleString()}</TableCell><TableCell/></TableRow>)}</TableBody></Table>
