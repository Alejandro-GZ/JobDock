...
describe("JobDetail",()=>{
  ...
  it("uses Overview/Metrics/Misc navigation and keeps attempt actions in their new locations",async()=>{
    ...
    await userEvent.click(screen.getByRole("tab",{name:"Metrics"}));const templates=await screen.findByRole("button",{name:"Templates"}),metricsTab=screen.getByRole("tab",{name:"Metrics"});expect(templates.compareDocumentPosition(metricsTab)&Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();expect(screen.getByRole("button",{name:"Edit dashboard"}).compareDocumentPosition(metricsTab)&Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();await userEvent.click(screen.getByRole("button",{name:"Edit dashboard"}));expect(screen.getByRole("button",{name:"Done"})).toBeTruthy();
