...
import { ArrowLeft, Download, RotateCcw, Square, Trash2 } from "lucide-react";
...
export function JobDetail({ user }: { user: User }) {
  ...
  const rerun = useMutation({ mutationFn: () => api.rerunJob(id), onSuccess: () => { toast.success("Job queued for rerun"); queryClient.invalidateQueries({ queryKey: ["job", id] }); queryClient.invalidateQueries({ queryKey: ["job-attempts", id] }); }, onError: (error: Error) => toast.error(error.message) });
  ...
  return <div className="relative space-y-5 lg:pl-10">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0">
        ...
        <div className="mt-1 flex min-w-0 items-center gap-2 font-mono text-xs text-muted-foreground">
          <span className="truncate">{current.id}</span><span aria-hidden="true" className="text-border">/</span>
          {attempts.data?.length ? <Select value={selectedAttemptID} onValueChange={setSelectedAttemptID}><SelectTrigger aria-label="Select attempt" className="h-auto w-auto shrink-0 gap-1 border-0 bg-transparent p-0 font-mono text-xs text-muted-foreground shadow-none hover:text-foreground focus:ring-0 [&>svg]:size-3"><SelectValue/></SelectTrigger><SelectContent align="start">{attempts.data.map(item => <SelectItem key={item.id} value={item.id}>Attempt {item.attempt_number}</SelectItem>)}</SelectContent></Select> : <span className="shrink-0">No attempts yet</span>}
        </div>
      </div>
...
function AttemptOverview({ job, selected, attempts }: { job: Job; selected?: JobAttempt; attempts: JobAttempt[] }) {
  ...
  return <div className="space-y-4">
    <dl className="grid overflow-hidden rounded-md border sm:grid-cols-2 lg:grid-cols-6">{facts.map(([label, value]) => <div key={label} className="border-b p-4 sm:border-r"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 break-all text-sm font-medium">{value}</dd></div>)}</dl>
