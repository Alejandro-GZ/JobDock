...
export function JobDetail({ user }: { user: User }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-3"><h1 className="text-xl font-semibold">{current.spec.name}</h1><StatusBadge status={current.status}/></div><p className="mt-1 font-mono text-xs text-muted-foreground">{current.id}</p></div><div className="flex gap-2"><Button asChild variant="outline" size="sm"><a href={`/api/v1/jobs/${id}/archive.zip`}><Download className="size-4"/>Download ZIP</a></Button>{!terminal.has(current.status) && <Button variant="outline" size="sm" onClick={() => stop.mutate()}><Square className="size-4"/>Stop</Button>}<AlertDialog><AlertDialogTrigger asChild><Button variant="destructive" size="sm" disabled={!terminal.has(current.status)}><Trash2 className="size-4"/>Delete</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete this job?</AlertDialogTitle><AlertDialogDescription>Logs and output files will be permanently removed. The audit tombstone remains.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => remove.mutate()}>Delete job</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div></div>
    <Tabs defaultValue="overview"><TabsList><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="logs">Logs</TabsTrigger><TabsTrigger value="metrics">Metrics</TabsTrigger><TabsTrigger value="events">Events</TabsTrigger></TabsList><TabsContent value="overview"><dl className="grid overflow-hidden rounded-md border sm:grid-cols-2 lg:grid-cols-5">{[["Image", current.spec.image], ["Node", current.assigned_node_id ?? "Unassigned"], ["Resources", `${current.spec.resources.cpu_millis / 1000} CPU · ${formatBytes(current.spec.resources.memory_bytes)} · ${current.spec.resources.gpu.count} GPU`], ["Inputs", `${current.spec.inputs?.length??0} files · ${formatBytes(current.spec.inputs?.reduce((total,input)=>total+input.size,0)??0)}`], ["Created", relative(current.created_at)]].map(([label, value]) => <div key={label} className="border-b p-4 sm:border-r"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 break-all text-sm font-medium">{value}</dd></div>)}</dl>{current.spec.inputs?.length?<div className="mt-4 divide-y rounded-md border">{current.spec.inputs.map(input=><div key={input.path} className="grid grid-cols-[1fr_auto] gap-3 p-3 text-xs"><span className="font-mono">{input.path}</span><span className="text-muted-foreground">{formatBytes(input.size)} · sha256:{input.sha256.slice(0,12)}</span></div>)}</div>:null}{current.failure_reason && <p className="mt-4 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{current.failure_reason}</p>}</TabsContent><TabsContent value="logs"><LiveLogs jobId={id}/></TabsContent><TabsContent value="metrics"><MetricsPanel job={current}/></TabsContent><TabsContent value="events"><div className="divide-y rounded-md border">{(events.data ?? []).map((event) => <div key={event.id} className="grid grid-cols-[150px_1fr_auto] gap-3 p-3 text-sm"><span className="font-mono text-xs">{event.type}</span><span className="truncate text-muted-foreground">{JSON.stringify(event.payload)}</span><time className="text-xs text-muted-foreground">{relative(event.created_at)}</time></div>)}</div></TabsContent></Tabs><Button asChild variant="link" className="px-0"><Link to="/jobs">Back to jobs</Link></Button></div>;
}
