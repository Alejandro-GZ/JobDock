...
describe("series helpers", () => {
  ...
  it("summarizes and zooms a numeric domain", () => {
    expect(numericSummary([{timestamp:1,value:3},{timestamp:2,value:1},{timestamp:3,value:5}])).toEqual({min:1,max:5,avg:3,last:5});
    expect(numericSummary([{timestamp:1,value:10,sampleCount:1},{timestamp:2,value:20,sampleCount:3}])?.avg).toBe(17.5);
  ...
  it("appends only new attempt-scoped live deltas", () => {
    ...
    const metricUpdate={cursor:5,attempt_id:"attempt",kind:"metrics" as const,captured_at:"2026-08-12T10:01:05Z",metrics:[{cursor:5,attempt_id:"attempt",name:"loss",value:1,step:2,captured_at:"2026-08-12T10:01:05Z",unit:"ratio",tags:["metric:loss","phase:train"],metadata:{split:"train"}}]};
    const nextMetrics=appendMetricUpdate(metrics,metricUpdate)!;
    expect(nextMetrics.cursor).toBe(5);expect(nextMetrics.series[0]).toMatchObject({last:1,min:1,max:2,avg:1.5,sample_count:2,unit:"ratio",tags:["metric:loss","phase:train"],metadata:{split:"train"}});expect(nextMetrics.series[0].points).toHaveLength(1);
