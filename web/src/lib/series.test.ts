...
describe("series helpers", () => {
  ...
  it("appends only new attempt-scoped live deltas", () => {
    ...
    expect(nextMetrics.cursor).toBe(5);expect(nextMetrics.series[0]).toMatchObject({last:1,min:1,max:2,sample_count:2,unit:"ratio",tags:["metric:loss","phase:train"],metadata:{split:"train"}});expect(nextMetrics.series[0].points).toHaveLength(1);
