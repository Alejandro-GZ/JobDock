...
const node = (overrides: Partial<Node>): Node => ({ id: "node-a", name: "Node A", status: "ONLINE", agent_version: "test", architecture: "amd64", docker_version: "test", cpu_total_millis: 8000, cpu_allocated_millis: 2000, memory_total_bytes: 16000, memory_allocated_bytes: 4000, workspace_free_bytes: 10000, labels: { zone: "lab" }, capabilities:[],cpu_packages:[],gpus: [{ uuid: "gpu-a", model: "RTX", vram_bytes: 8000, allocated: false }], gpu_discovery: { status: "available" }, last_heartbeat: new Date().toISOString(), ...overrides });
