...
describe("dashboard widget model",()=>{
  ...
  it("restores versioned widget data and safely rejects invalid saved layouts",()=>{
    ...
    expect(restored?.[0]).not.toHaveProperty("table_sort_order");
    expect(restored?.[0]).not.toHaveProperty("residual_x_axis");
    expect(restored?.[0]).not.toHaveProperty("shap_mode");
    expect(restored?.[0]).not.toHaveProperty("projection_color_by");
