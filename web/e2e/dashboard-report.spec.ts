...
test("renders a multi-dashboard report without network access", async ({ page }, testInfo) => {
  ...
  const manifest={schema_version:1,jobdock_version:"v-test",generated_at:generatedAt,theme:"dark",job:{id:"job-1",name:"Offline training report",status:"SUCCEEDED",created_at:"2026-08-29T11:57:00Z",started_at:"2026-08-29T11:58:00Z",finished_at:generatedAt},attempt:{id:"attempt-1",attempt_number:3,status:"SUCCEEDED",created_at:"2026-08-29T11:57:00Z",started_at:"2026-08-29T11:58:00Z",finished_at:generatedAt},dashboards:[
    {id:"overview",name:"Training overview",is_default:true,schema_version:1,updated_at:generatedAt,config:{appearance:{schema_version:1,palette:{id:"ocean",version:1}},widgets:[{id:"loss",type:"lineplot",title:"Loss",grid_columns:12,size:{columns:8,rows:6},position:{x:0,y:0},sources:[{kind:"metric",name:"loss"},{kind:"metric",name:"accuracy"}],x_axis:"step"},{id:"progress",type:"progress",grid_columns:12,size:{columns:4,rows:6},position:{x:8,y:0},sources:[{kind:"progress",name:"progress"}]}]}},
  ...
  await expect(page.getByRole("img",{name:"JobDock"})).toBeVisible();
  await expect(page.getByText("Offline training report")).toBeVisible();
  await expect(page.getByText("2m 0s")).toBeVisible();
  await expect(page.locator('[data-report-status="SUCCEEDED"]')).toHaveText("SUCCEEDED");
  ...
  const plot=page.getByRole("img",{name:/Loss lineplot with 4 points/});
  await expect(plot).toBeVisible();
  await plot.hover({position:{x:500,y:120}});
  await expect(page.locator("[data-plot-tooltip]")).toContainText("loss");
  await expect(page.locator("[data-plot-tooltip]")).toContainText("accuracy");
